...
class LLMJudge:
    ...
    """
        ...
        try:
            ...
            for criterion in rubric:
                score = raw_scores.get(criterion, 2.5)

                # Convert 1-5 scale to 0-1 scale
                scores[criterion] = max(0.0, min(1.0, float(score) / 5.0))
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Positional bias:
        # Compare the first score with the average of the remaining scores.
        first_scores = []
        other_scores = []
        ...
        if first_scores and other_scores:
            ...
            positional_bias = first_avg > other_avg
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Preserve the original QAPair object.
            result.qa_pair = pair
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avg_faithfulness = average(
            new_results, "faithfulness"
        )
        new_avg_relevance = average(
            new_results, "relevance"
        )
        new_avg_completeness = average(
            new_results, "completeness"
        )
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Improve prompt clarity and intent detection to keep answers relevant"
            )

        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Improve intent detection and topic filtering to prevent off-topic answers"
            )

        if len(suggestions) < 3:
            suggestions.extend([
                "Increase chunk size in RAG pipeline to reduce context fragmentation",
                "Review retrieval ranking to improve the quality of retrieved context",
                "Add regression tests for recurring failure cases",
            ])
